...
static std::string _rosbag;
static std::string _map_path;
...
static double _voxel_grid_size = 2.0;
...
//  pcl::PointCloud<pcl::PointXYZ>::Ptr output_cloud(new pcl::PointCloud<pcl::PointXYZ>);
  ...
  std::cout << "finished setup ndt map." << std::endl;
}
...
{
  // if (map_loaded == 0)
  if (points_map_num != map.points.size())
//  if (1)
  {
    std::cout << "Update points_map." << std::endl;

    points_map_num = map.points.size();
...
static void voxel_grid_filter(const sensor_msgs::PointCloud2::ConstPtr& input, const double voxel_leaf_size, pcl::PointCloud<pcl::PointXYZ>& filtered_scan)
{
...
//  {
    ...
    pcl::copyPointCloud(*filtered_scan_ptr, filtered_scan);
...
std::vector<std::string> get_pcd_list(std::string path)
{
  std::string ext = ".pcd";

  std::vector<std::string> ret;
  struct dirent **namelist;
  int r = scandir(path.c_str(), &namelist, NULL, NULL);
  if(r == -1){
    std::cout << "Error: scan directory " << path << std::endl;
  }

  for(int i = 0; i < r; i++){
    std::string pcd = path + namelist[i]->d_name;
    if(pcd.compare(pcd.size() - ext.size(), ext.size(), ext) == 0){
      ret.push_back(pcd);
    }
    free(namelist[i]);
  }
...
static void load_pcd(std::string path, pcl::PointCloud<pcl::PointXYZ> &map)
{
  map.clear();
  // get filenames in map_path;
  std::vector<std::string> pcd_list = get_pcd_list(path);
  for(auto itr = pcd_list.begin(); itr != pcd_list.end(); ++itr){
    pcl::PointCloud<pcl::PointXYZ> cloud;
    if(pcl::io::loadPCDFile<pcl::PointXYZ>(*itr, cloud) == -1){
      std::cout << "Failed to load " << *itr << "." << std::endl;
    }
    std::cout << "Loaded " << cloud.points.size() << " points from " << *itr << "." << std::endl;
    map += cloud;
  }
}
...
//  node_status_publisher_ptr_->CHECK_RATE("/topic/rate/points_raw/slow",8,5,1,"topic points_raw subscribe rate low.");
  std::chrono::time_point<std::chrono::system_clock> voxel_grid_start, voxel_grid_end, matching_start, matching_end, align_start, align_end, getFitnessScore_start, getFitnessScore_end;
  static double voxel_grid_time = 0.0;
  ...
  if (map_loaded == 1 && init_pos_set == 1)
  {
    ...
    int points_num_original, points_num_filtered;
...
//    pcl::fromROSMsg(*input, filtered_scan);
    voxel_grid_start = std::chrono::system_clock::now();
    voxel_grid_filter(input, _voxel_grid_size, filtered_scan);
    voxel_grid_end = std::chrono::system_clock::now();
    voxel_grid_time = std::chrono::duration_cast<std::chrono::microseconds>(voxel_grid_end - voxel_grid_start).count() / 1000.0;

    pcl::PointCloud<pcl::PointXYZ>::Ptr filtered_scan_ptr(new pcl::PointCloud<pcl::PointXYZ>(filtered_scan));
    points_num_original = input->width;
    points_num_filtered = filtered_scan_ptr->size();
...
int main(int argc, char** argv)
{
  ...
  private_nh.getParam("rosbag", _rosbag);

  private_nh.getParam("resolution", ndt_res);
  private_nh.getParam("step_size", step_size);
  private_nh.getParam("trans_epsilon", trans_eps);
  private_nh.getParam("max_iterations", max_iter);
  private_nh.getParam("voxel_grid_size", _voxel_grid_size);

  private_nh.getParam("init_x", initial_pose.x);
  private_nh.getParam("init_y", initial_pose.y);
  private_nh.getParam("init_z", initial_pose.z);
  private_nh.getParam("init_roll", initial_pose.roll);
  ...
  ofs_config << "method_type: " << (int)_method_type << std::endl
             ...
             << "queue_size: " << _queue_size << std::endl
             << "get_height: " << _get_height << std::endl
             << "use_local_transform: " << _use_local_transform << std::endl
             << "use_imu: " << _use_imu << std::endl
             << "use_odom: " << _use_odom << std::endl
             << "imu_upside_down: " << _imu_upside_down << std::endl
             << "imu_topic: " << _imu_topic << std::endl
             << "map_path: " << _map_path << std::endl
             << "rosbag: " << _rosbag << std::endl
             << "resolution: " << ndt_res << std::endl
             << "step_size: " << step_size << std::endl
             << "trans_epsilon: " << trans_eps << std::endl
             << "max_iterations: " << max_iter << std::endl
             << "voxel_grid_size: " << _voxel_grid_size << std::endl
             << "init_x: " << initial_pose.x << std::endl
             << "init_y: " << initial_pose.y << std::endl
  ...
  // Updated in initialpose_callback or gnss_callbac
  /*
  initial_pose.x = 0.0;
  initial_pose.y = 0.0;
  initial_pose.z = 0.0;
...
*/
  ...
  map_pub = nh.advertise<sensor_msgs::PointCloud2>("/points_map", 10, true);
...
//  pthread_create(&thread, NULL, thread_func, NULL);
  ...
  // read map map
  load_pcd(_map_path, map);
  std::cout << "map: " << map.points.size() << std::endl;
  // publish map
  ...
  rosbag::Bag bag;
  std::cout << "rosbag: " << _rosbag << std::endl;
  bag.open(_rosbag, rosbag::bagmode::Read);
  ...
  rosbag::View view_points_raw(bag, rosbag::TopicQuery("/points_raw"));
  int msg_num = view_points_raw.size();
  std::cout << "msg_num: " << msg_num << std::endl;
  int num = 1;
  foreach(rosbag::MessageInstance const m, view){
    sensor_msgs::PointCloud2::ConstPtr points_msg = m.instantiate<sensor_msgs::PointCloud2>();
    if(points_msg != NULL){
      points_callback(points_msg);
      std::cout << num << " / " << msg_num << std::endl;
      num++;
    }
    sensor_msgs::Imu::Ptr imu_msg = m.instantiate<sensor_msgs::Imu>();
    if(imu_msg != NULL){
